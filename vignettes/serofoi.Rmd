...
To assess how well the mathematical representation of the different *FoI* models describes the seroprevalence data, we implement a fitting process that relies on a Bayesian framework. The priors and upper priors for each model indicate the assumed trajectory of the *FoI* (either constant or time-varying). serofoi provides a set of Bayesian comparison methods for the user to choose between various possible FoI models (trajectories) that best fit the seroprevalence data. For details see [FoI Models](https://trace-lac.github.io/serofoi/articles/foi_models.html).
...
The integrated dataset `chagas2012` provides a minimal example of the input of the package. The following code can be used to implementat and visualise a basic constant model:
