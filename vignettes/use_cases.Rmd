...
chagas2014 <- readRDS("../data/chagas2014.RDS") # This can be removed once data is incorporated
chagas2014 <- chagas2014 %>% filter(age_min < 30)
head(chagas2014, 5)

```

After preparing the data we can run the three different models available on serofoi, one constant and two time-varying model.

```{r three models, include = TRUE, errors = FALSE, warning = FALSE, message=FALSE }

chagas2014p <- prepare_data(chagas2014)

m1_cha <- run_model(model_data = chagas2014p,
                     model_name = "constant_foi_bi",
                     n_iters = 500, 
                     n_thin = 2)

m2_cha <- run_model(model_data = chagas2014p,
                     model_name = "continuous_foi_normal_bi",
                     n_iters = 1500, 
                     n_thin = 2)

m3_cha <- run_model(model_data = chagas2014p,
                     model_name = "continuous_foi_normal_log",
                     n_iters = 1500, 
                     n_thin = 2)
...
Now, we can plot the results of the three models to compare (Figure 1). As shown in @Cucunubá2017, interventions for Chagas control have been ongoing from the 1980s in Colombia, reflecting a marked reduction in sero-prevalence in children. The serofoi models are able to detect this chance suggesting . (consistent with an epidemic).

```{r, model_comp_cha, include=TRUE, echo=TRUE, fig.width = 5, fig.height=5, warning=FALSE, message=FALSE, message=FALSE}
p1_cha <- plot_model(m1_cha, size_text = 15, max_lambda = 0.02)
p2_cha <- plot_model(m2_cha, size_text = 15, max_lambda = 0.02)
p3_cha <- plot_model(m3_cha, size_text = 15, max_lambda = 0.02)
...
```{r, model_comp_veev, include=TRUE, echo=TRUE, fig.width = 5, fig.height=5, warning=FALSE, message=FALSE, message=FALSE}
...
```{r, plot_chik, include=TRUE, echo=TRUE, fig.width = 5, fig.height=5, warning=FALSE, message=FALSE, message=FALSE}
