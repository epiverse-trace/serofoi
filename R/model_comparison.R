...
extract_and_save <- function(res_file_1, res_file_2, res_file_3,
  ...
  foi_cent_est3 <- data.frame(year  = RealYexpo,
                              ...
                              upper = apply(foi_1, 2, function(x) quantile(x, 0.9)),
                              median = apply(foi_1, 2, function(x) quantile(x, 0.5))) %>%
    mutate(best= 'best3', name_model = best_model_3)
  ...
  foi_2_post_1000s <- dplyr::sample_n(as.data.frame(foi_2), size = 1000) %>% mutate(best = 'best3', name_model = best_model_3)
  ...
  prev3 <- res_file_3$prev_expanded %>% mutate(best = 'best3', name_model = best_model_3)
