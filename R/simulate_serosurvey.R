...
#' Generate probabilities of seropositivity by age based on an age-varying FOI
#' model.
#'
#' This function calculates the probabilities of seropositivity by age based on
...
#'
#' This function calculates the probabilities of seropositivity by age based on
#' an age-and-time-varying FOI model. It takes into account the FOI and the
#' rate of seroreversion.
#'
#' @param foi A dataframe containing the force of infection (FOI) values for
#' different ages. It should have three columns: 'year', 'age' and 'foi'.
#' @param seroreversion_rate A non-negative numeric value representing the rate
#' of seroreversion.
#'
#' @return A dataframe with columns 'age' and 'seropositivity'.
...
#' survey_features and age_df. This dataframe has columns including 'age' and
#' 'overall_sample_size'.
...
#' the provided survey features.
#' It first creates age bins, assigns each individual in the survey features to
#' an age bin, calculates the overall sample size by group, and then generates
#' random sample sizes for each age group.
#' Finally, it returns a dataframe with the random sample sizes for each
#' individual age.
#'
...
sample_size_by_individual_age_random <- function(survey_features) {
  ...
  for (i in seq_along(ages)) {
    # Find the index of the row in survey_features where age falls within range
...
#' model, optionally including seroreversion.
#' This function allows construction of serosurveys with binned age groups,
#' and it generates uncertainty in the distribution of a sample size within an
#' age bin through multinomial sampling.
#'
#' @inheritParams probability_seropositive_time_model_by_age
...
#' age bin through multinomial sampling.
#'
#' @inheritParams probability_seropositive_age_model_by_age
#' @param survey_features A dataframe containing information about the binned
...
#'
#' @inheritParams probability_seropositive_age_and_time_model_by_age
#' @param survey_features A dataframe containing information about the binned
#' age groups and sample sizes for each.
#' It should contain columns: ['age_min', 'age_max', 'sample_size'].
#'
